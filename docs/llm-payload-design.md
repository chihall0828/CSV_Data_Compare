...
**LLM呼び出し（fetch）・依存追加は行いません。** Phase L1 のpayload生成関数とvalidationのみ実装済みです。
...
4. Ollama接続UIの検討（Phase L2） — 未着手
5. Markdown形式の考察生成（Phase L3） — 未着手

いきなりOllama fetchは実装しません。Phase L2以降も本方針の禁止事項（API key・依存追加・生データ送信）を維持します。
